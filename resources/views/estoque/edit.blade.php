...
<div class="max-w-5xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow p-6 max-w-4xl mx-auto">
        ...
        <form method="POST" action="{{ route('estoque.update', $estoque) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="codigo_barras" required
                        ...
                        value="{{ old('codigo_barras', $estoque->codigo_barras) }}">
                    @error('codigo_barras')
            ...
            <div class="grid grid-cols-3 gap-5">
                ...
                <div>
                    ...
                    <select name="unidade" required
                        ...
                        <option value="UN" {{ old('unidade', $estoque->unidade) == 'UN' ? 'selected' : '' }}>Unidade (UN)</option>
                        <option value="CX" {{ old('unidade', $estoque->unidade) == 'CX' ? 'selected' : '' }}>Caixa (CX)</option>
                        <option value="KG" {{ old('unidade', $estoque->unidade) == 'KG' ? 'selected' : '' }}>Quilograma (KG)</option>
                        <option value="L" {{ old('unidade', $estoque->unidade) == 'L' ? 'selected' : '' }}>Litro (L)</option>
                        <option value="M" {{ old('unidade', $estoque->unidade) == 'M' ? 'selected' : '' }}>Metro (M)</option>
                        <option value="M2" {{ old('unidade', $estoque->unidade) == 'M2' ? 'selected' : '' }}>Metro Quadrado (M²)</option>
                    </select>
                    @error('unidade')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-3 gap-5">
                ...
                <div>
                    ...
                    <input type="number" name="estoque_maximo" required
                        ...
                        value="{{ old('estoque_maximo', $estoque->estoque_maximo) }}">
                ...
                <div class="flex items-end">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="controla_estoque" value="1" 
                            class="rounded w-4 h-4" {{ old('controla_estoque', $estoque->controla_estoque) ? 'checked' : '' }}>
            ...
            <div>
                ...
                <textarea name="descricao" rows="3"
                    class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-400">{{ old('descricao', $estoque->descricao) }}</textarea>
            </div>
