    ...
    <div class="bg-white rounded-xl shadow">
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center gap-4">
            ...
            <div class="flex items-center gap-3 flex-1">
                <!-- Busca -->
                <form method="GET" action="{{ route('contas_receber.index') }}" class="flex-1 max-w-md">
                    <div class="relative">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('contas_receber.index', ['status' => 'pendente']) }}"
                        class="px-4 py-2 text-sm rounded-lg transition {{ !request('status') ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
                        Pendentes
                    </a>
                    <a href="{{ route('contas_receber.index', ['status' => 'pago']) }}"
            ...
            <a href="{{ route('contas_receber.create') }}" class="flex items-center gap-2 px-4 py-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition font-medium">
        ...
        <table class="w-full">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-700">RECEBIMENTO</th>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-6 py-4">
                        <p class="text-sm font-medium text-gray-900">{{ $conta->cliente->nome ?? 'N/A' }}</p>
                    </td>
                    <td class="px-6 py-4">
                        <p class="text-sm font-semibold text-emerald-600">R$ {{ number_format($conta->valor, 2, ',', '.') }}</p>
                    </td>
                    <td class="px-6 py-4">
                        @php
                    ...
                    <td class="px-6 py-4 text-center">
                        @if($conta->status === 'pago')
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-emerald-50 text-emerald-700">✓ Recebido</span>
                        @else
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-yellow-50 text-yellow-700">⏱ Pendente</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('contas_receber.edit', $conta) }}" class="p-2 text-gray-600 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition">
